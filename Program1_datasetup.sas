...
array adhbin_aFV(*) 		adhbinFV0 - adhbinFV15;
array cum_adhbin(*)		adhbin27-adhbin42;
array avgadh_t(*)		avg_adh0 - avg_adh15;
...
if     		. < adhFV0 < 80 then adhbinFV0 = 1;
else if 	   adhFV0 >=80 then adhbinFV0 = 0;
else if 	   adhFV0 = .  then adhbinFV0 = .;

do i=2 to 15;
       	if   		  . < adhFV(i) < 80 then adhbin_aFV(i) = 1;
      	else if 	   adhFV(i) >=80 then adhbin_aFV(i) = 0;
      	else if 	   adhFV(i) = .  then adhbin_aFV(i) = .;
end;

avg_adh0 = adhbinFV0;

do i = 2 to 15;
	avgadh_t(i)=(avgadh_t(i-1)*(i-1)+adhbin_aFV(i))/i; 
end;
...
do i = 1 to 15;
	...
	if i < 4 then do;
		if invdth = 0 then do;
			...
			adhr_t = adhbin_a(i);
			adhr_tc = adhbin_aFV(i);
			cum_adhr_t = cum_adhbin(i);
			
			avg_adh_t = avgadh_t(i);

			death = 0;

			if i ge 2 then do; adhr_t1 = adhbin_a(i-1); adhr_tc1 = adhbin_aFV(i-1);end;
			if i ge 3 then do; adhr_t2 = adhbin_a(i-2); adhr_tc2 = adhbin_aFV(i-2);end;
			
			if i ge 2 then cum_adhr_t1 = cum_adhbin(i-1);
			if i ge 3 then cum_adhr_t2 = cum_adhbin(i-2);

			if i ge 2 then avg_adh_t1 = avgadh_t(i-1);
			if i ge 3 then avg_adh_t2 = avgadh_t(i-2);
		end;

		else if i > invdth-1 then do;
			cens = .; 
			p_cens0 = .;
			adhr_t = .;
			cum_adhr_t = .;
			...
			if invdth - 1 = i then death = 1;	
			else if  invdth - 1 > i then death = 0;
		...
		if indic(i) = 0 	then do; 
			...
			if invdth - 1 = i then death = 1;	
			else if invdth = 0 or invdth - 1 > i then death = 0;
		end;
		...
		if death in (0,1) then do;		
			...
			adhr_tc1 = adhbin_aFV(i-1);
			adhr_tc2 = adhbin_aFV(i-2);
			adhr_tc3 = adhbin_aFV(i-3);	

			cum_adhr_t  = cum_adhbin(i);

			cum_adhr_t1 = cum_adhbin(i-1);
			cum_adhr_t2 = cum_adhbin(i-2);
			cum_adhr_t3 = cum_adhbin(i-3);	

			avg_adh_t = avgadh_t(i);

			avg_adh_t1 = avgadh_t(i-1);
			avg_adh_t2 = avgadh_t(i-2);
			avg_adh_t3 = avgadh_t(i-3);
	...
	Adhx15Bin adhx15 itr entrydate death adhFV0 -adhFV25

 	adhbin0  adhr_t adhr_t1 adhr_t2 adhr_t3  adhr_tc adhr_tc1 adhr_tc2 adhr_tc3  cens p_cens0  adh_measure

	cum_adhr_t cum_adhr_t1 cum_adhr_t2 cum_adhr_t3 adhbin27-adhbin42

	avg_adh_t avg_adh_t1  avg_adh_t2  avg_adh_t3 adhbinFV0 - adhbinFV15  avg_adh0 - avg_adh15
